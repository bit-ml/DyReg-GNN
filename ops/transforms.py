...
class DistillGroupMultiScaleCrop(object):
    ...
    def reverse_crop(p_h, p_w, resize_ratio_h, resize_ratio_w, offset_h, offset_w):
        abs_p_h = x_h / resize_ratio_h + offset_h
        abs_p_w = x_w / resize_ratio_w + offset_w

    def __call__(self, img_group, crop_params=None):
...
class GroupShiftCrop(object):
    def __init__(self, size,dh=0,dw=0):
        if isinstance(size, numbers.Number):
            self.size = (int(size), int(size))
        else:
            self.size = size
...
class DistillGroupNormalize(object):
    def __init__(self, mean, std):
        self.mean = mean
        self.std = std

    def __call__(self, fct_tensor):
...
class DistillStack(object):
    def __init__(self, roll=False):
        self.roll = roll

    def __call__(self, fct_img_group):
...
class DistillToTorchFormatTensor(object):
    """ Converts a PIL.Image (RGB) or numpy.ndarray (H x W x C) in the range [0, 255]
    to a torch.FloatTensor of shape (C x H x W) in the range [0.0, 1.0] """
    def __init__(self, div=True):
        self.div = div

    def __call__(self, fct_pic):
        fct, pic = fct_pic
        if isinstance(pic, np.ndarray):
            # handle numpy array
            img = torch.from_numpy(pic).permute(2, 0, 1).contiguous()
